...
return function(){
    ...
    check.run( 'after( function )', function(){
            var links = E.query( '#afterList3 a' );
            E( '#afterList4 span' ).after(function( index ){
                return links[ index ];
            });
            return E( '#afterList4 a' ).length;
        })
        .equal( 2 );
    ...
    // appendTo 
    check.run( 'appendTo( function )', function(){
            var spans = E.query( '#appendToList2 span' );
            E( '#appendToList1 a' ).appendTo(function( index ){
                return spans[ index ];
            });
            
            return E( '#appendToList2 a' ).length;
        })
        .equal( 2 );
    ...
    // every
    check.run( 'every', function(){
            return E( '#queryList1 li' ).every(function(){
    ...
    // some
    check.run( 'some', function(){
            return E( '#queryList1 li' ).some(function(){
                return this.className === 'item3';
            });
        })
        .equal( true );
    ...
    check.run( 'html( function )', function(){
            E( '#htmlList1 dd' ).html(function( i, content ){
                return i + content;
            });
            return E( '#htmlList1 dd' ).html().trim().toLowerCase();
        })
        .equal( '0dd1' );
    ...
    // map
    check.run( 'map', function(){
            return E( '#queryList1 li' ).map(function(){
    ...
    check.run( 'text( function )', function(){
            E( '#queryList1 li' ).text(function( i, content ){
                return i + content;
            });
            return E( '#queryList1 li:first-child' ).text();
        })
